...
inline Result RendererSetTextureDefaultShadeCount(IDirect3DRM2* pRenderer, unsigned int shadeCount)
{
	return ResultVal(pRenderer->SetDefaultTextureShades(shadeCount));
}

// FUNCTION: BETA10 0x1016b020
inline Result RendererSetTextureDefaultColorCount(IDirect3DRM2* pRenderer, unsigned int colorCount)
{
...
inline unsigned int DeviceGetTrianglesDrawn(IDirect3DRMDevice2* pDevice)
{
#ifdef MINIWIN
	return 0;
#else
...
)
{
	...
	for (int i = 0; i < faceCount; i++) {
		if (((((unsigned short*) p_faceIndices)[2 * i + 1]) >> 0x0f) & 0x01) {
			unsigned int j = 3 * ((unsigned short*) p_faceIndices)[2 * i];
