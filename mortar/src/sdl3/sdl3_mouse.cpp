...
{
	switch (id) {
	case MORTAR_SYSTEM_CURSOR_DEFAULT: 		return SDL_SYSTEM_CURSOR_DEFAULT;
	case MORTAR_SYSTEM_CURSOR_WAIT:			return SDL_SYSTEM_CURSOR_WAIT;
	case MORTAR_SYSTEM_CURSOR_NOT_ALLOWED: 	return SDL_SYSTEM_CURSOR_NOT_ALLOWED;
	default:
		abort();
	}
}
...
SDL_MouseButtonFlags mousebuttonflags_mortar_to_sdl3(MORTAR_MouseButtonFlags flags)
{
	...
	if (flags & MORTAR_BUTTON_LMASK) 	{ result |= SDL_BUTTON_LMASK; }
	if (flags & MORTAR_BUTTON_MMASK) 	{ result |= SDL_BUTTON_MMASK; }
	if (flags & MORTAR_BUTTON_RMASK) 	{ result |= SDL_BUTTON_RMASK; }
	if (flags & MORTAR_BUTTON_X1MASK) 	{ result |= SDL_BUTTON_X1MASK; }
	if (flags & MORTAR_BUTTON_X2MASK) 	{ result |= SDL_BUTTON_X2MASK; }
	return result;
}
...
MORTAR_MouseButtonFlags mousebuttonflags_sdl3_to_mortar(SDL_MouseButtonFlags flags)
{
	...
	if (flags & SDL_BUTTON_LMASK) 	{ result |= MORTAR_BUTTON_LMASK; }
	if (flags & SDL_BUTTON_MMASK) 	{ result |= MORTAR_BUTTON_MMASK; }
	if (flags & SDL_BUTTON_RMASK) 	{ result |= MORTAR_BUTTON_RMASK; }
	if (flags & SDL_BUTTON_X1MASK) 	{ result |= MORTAR_BUTTON_X1MASK; }
	if (flags & SDL_BUTTON_X2MASK) 	{ result |= MORTAR_BUTTON_X2MASK; }
	return result;
}
