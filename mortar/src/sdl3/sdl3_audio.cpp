...
{
	switch (format) {
	case MORTAR_AUDIO_F32: return SDL_AUDIO_F32;
	default:
		abort();
	}
}

MORTAR_AudioStream * MORTAR_EX_OpenAudioPlaybackDevice(const MORTAR_AudioSpec *spec, MORTAR_AudioStreamCallback callback, void *userdata)
{
	...
	return (MORTAR_AudioStream *)SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &sdl3_spec, (SDL_AudioStreamCallback) callback, userdata);
}
