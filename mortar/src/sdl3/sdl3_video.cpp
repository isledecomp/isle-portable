...
MORTAR_WindowFlags MORTAR_GetWindowFlags(MORTAR_Window *mortar_window)
{
	...
	if (sdl3_flags & SDL_WINDOW_FULLSCREEN) {	mortar_flags |= MORTAR_WINDOW_FULLSCREEN;}
	if (sdl3_flags & SDL_WINDOW_BORDERLESS) {	mortar_flags |= MORTAR_WINDOW_BORDERLESS;}
	if (sdl3_flags & SDL_WINDOW_RESIZABLE) {	mortar_flags |= MORTAR_WINDOW_RESIZABLE;}
	return mortar_flags;
}
...
MORTAR_DisplayMode **MORTAR_GetFullscreenDisplayModes(MORTAR_DisplayID displayID, int *count)
{
	...
	uint8_t *buffer = (uint8_t *) SDL_malloc(sizeof(MORTAR_DisplayMode *) * (sdl3_count + 1) + sizeof(MORTAR_DisplayMode) * sdl3_count);
	MORTAR_DisplayMode **mortar_modes_pointers = (MORTAR_DisplayMode **) buffer;
	MORTAR_DisplayMode *mortar_modes_data = (MORTAR_DisplayMode *) (buffer + sizeof(MORTAR_DisplayMode *) * (sdl3_count + 1));
...
bool MORTAR_GL_SetAttribute(MORTAR_GLAttr attr, int value)
{
	...
	switch (attr) {
	case MORTAR_GL_DOUBLEBUFFER:				sdl3_attr = SDL_GL_DOUBLEBUFFER; break;
	case MORTAR_GL_DEPTH_SIZE:					sdl3_attr = SDL_GL_DEPTH_SIZE; break;
	case MORTAR_GL_MULTISAMPLEBUFFERS:			sdl3_attr = SDL_GL_MULTISAMPLEBUFFERS; break;
	case MORTAR_GL_MULTISAMPLESAMPLES:			sdl3_attr = SDL_GL_MULTISAMPLESAMPLES; break;
	case MORTAR_GL_CONTEXT_MAJOR_VERSION:		sdl3_attr = SDL_GL_CONTEXT_MAJOR_VERSION; break;
	case MORTAR_GL_CONTEXT_MINOR_VERSION:		sdl3_attr = SDL_GL_CONTEXT_MINOR_VERSION; break;
	case MORTAR_GL_CONTEXT_PROFILE_MASK:		sdl3_attr = SDL_GL_CONTEXT_PROFILE_MASK; break;
	default:
		abort();
	}
...
bool MORTAR_GetClosestFullscreenDisplayMode(MORTAR_DisplayID displayID, int w, int h, float refresh_rate, bool include_high_density_modes, MORTAR_DisplayMode *closest)
{
	SDL_DisplayMode closest_sdl3;
	if (!SDL_GetClosestFullscreenDisplayMode((SDL_DisplayID)displayID, w, h, refresh_rate, include_high_density_modes, &closest_sdl3)) {
		return false;
	}
...
void * MORTAR_EXT_GetWindowProperty(MORTAR_Window *mortar_window, MORTAR_WindowProperty key, void *default_value)
{
	...
	case MORTAR_WINDOW_PROPERTY_HWND:
		return SDL_GetPointerProperty(SDL_GetWindowProperties(window_mortar_to_sdl3(mortar_window)), SDL_PROP_WINDOW_WIN32_HWND_POINTER, nullptr);
	default:
