...
{
	switch (scaleMode) {
	case MORTAR_SCALEMODE_NEAREST:	return SDL_SCALEMODE_NEAREST;
	case MORTAR_SCALEMODE_LINEAR:	return SDL_SCALEMODE_LINEAR;
...
static MORTAR_Surface *create_mortar_surface_from_sdl3(SDL_Surface *sdl3_surface)
{
	...
	MORTAR_SDL_Surface *mortar_surface = (MORTAR_SDL_Surface *) SDL_calloc(sizeof(MORTAR_SDL_Surface), 1);
	if (!mortar_surface) {
...
{
	return create_mortar_surface_from_sdl3(SDL_CreateSurfaceFrom(width, height, pixelformat_mortar_to_sdl3(format), pixels, pitch));
}
...
void MORTAR_DestroySurface(MORTAR_Surface *mortar_surface)
{
	...
	if (mortar_sdl_surface->mortar_palette) {
		...
		if (mortar_sdl_surface->mortar_palette->sdl3_palette->refcount == 1) {
			SDL_free(mortar_sdl_surface->mortar_palette);
		}
	}
...
bool MORTAR_BlitSurface(MORTAR_Surface *mortar_src, const MORTAR_Rect *mortar_srcrect, MORTAR_Surface *mortar_dst, const MORTAR_Rect *mortar_dstrect)
{
	...
	return SDL_BlitSurface(surface_mortar_to_sdl3(mortar_src), sdl3_srcrect, surface_mortar_to_sdl3(mortar_dst), sdl3_dstrect);
}

bool MORTAR_SetSurfacePalette(MORTAR_Surface * mortar_surface, MORTAR_Palette *mortar_palette)
{
	MORTAR_SDL_Surface *mortar_sdl_surface = (MORTAR_SDL_Surface *)mortar_surface;
	if (mortar_sdl_surface && mortar_sdl_surface->mortar_palette) {
		mortar_sdl_surface->mortar_palette->sdl3_palette->refcount -= 1;
	}
...
{
	MORTAR_Surface *mortar_result_surface = create_mortar_surface_from_sdl3(SDL_ConvertSurface(surface_mortar_to_sdl3(mortar_surface), pixelformat_mortar_to_sdl3(format)));
	if (mortar_result_surface && mortar_result_surface->format == MORTAR_PIXELFORMAT_INDEX8) {
		...
		if (mortar_result_sdl_surface->mortar_palette->sdl3_palette) {
...
bool MORTAR_BlitSurfaceScaled(MORTAR_Surface *mortar_src, const MORTAR_Rect *mortar_srcrect, MORTAR_Surface *mortar_dst, const MORTAR_Rect *mortar_dstrect, MORTAR_ScaleMode mortar_scaleMode)
{
	SDL_Rect sdl3_srcrect_storage;
	SDL_Rect sdl3_dstrect_storage;
