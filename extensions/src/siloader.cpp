...
{
	char* files = MORTAR_strdup(options["si loader:files"].c_str());
	char* saveptr;

	for (char* file = MORTAR_strtok_r(files, ",\n\r ", &saveptr); file; file = MORTAR_strtok_r(NULL, ",\n\r ", &saveptr)) {
		SiLoader::files.emplace_back(file);
	}

	char* directives = MORTAR_strdup(options["si loader:directives"].c_str());

	for (char* directive = MORTAR_strtok_r(directives, ",\n\r ", &saveptr); directive;
		 directive = MORTAR_strtok_r(NULL, ",\n\r ", &saveptr)) {
		SiLoader::directives.emplace_back(directive);
	}

	MORTAR_free(files);
	MORTAR_free(directives);
}
...
std::optional<MxResult> SiLoader::HandleStart(MxDSAction& p_action)
{
	...
	if (p_action.GetExtraLength() == 0 || !MORTAR_strstr(p_action.GetExtraData(), prependedMarker)) {
...
bool SiLoader::LoadFile(const char* p_file)
{
	...
	if (si.Read(path.GetData(), si::Interleaf::ObjectsOnly) != si::Interleaf::ERROR_SUCCESS) {
		...
		if (si.Read(path.GetData(), si::Interleaf::ObjectsOnly) !=
			...
			MORTAR_Log("Could not parse SI file %s", p_file);
			return false;
		}
	}
...
bool SiLoader::LoadDirective(const char* p_directive)
{
	...
	else if (MORTAR_sscanf(p_directive, "RemoveWith:%255[^:;]%*[:;]%u%*[:;]%255[^:;]%*[:;]%u", originAtom, &originId, targetAtom, &targetId) == 4) {
		removeWith.emplace_back(
			StreamObject{MxAtomId{originAtom, e_lowerCase2}, originId},
			StreamObject{MxAtomId{targetAtom, e_lowerCase2}, targetId}
		);
	}
	else if (MORTAR_sscanf(p_directive, "Replace:%255[^:;]%*[:;]%u%*[:;]%255[^:;]%*[:;]%u", originAtom, &originId, targetAtom, &targetId) == 4) {
		replace.emplace_back(
			StreamObject{MxAtomId{originAtom, e_lowerCase2}, originId},
			StreamObject{MxAtomId{targetAtom, e_lowerCase2}, targetId}
		);
	}
	else if (MORTAR_sscanf(p_directive, "Prepend:%255[^:;]%*[:;]%u%*[:;]%255[^:;]%*[:;]%u", originAtom, &originId, targetAtom, &targetId) == 4) {
		prepend.emplace_back(
			StreamObject{MxAtomId{targetAtom, e_lowerCase2}, targetId},
			StreamObject{MxAtomId{originAtom, e_lowerCase2}, originId}
		);
	}
	else if (MORTAR_sscanf(p_directive, "FullScreenMovie:%255[^:;]%*[:;]%u", originAtom, &originId) == 2) {
		fullScreenMovie.emplace_back(StreamObject{MxAtomId{originAtom, e_lowerCase2}, originId});
	}
	else if (MORTAR_sscanf(p_directive, "Disable3d:%255[^:;]%*[:;]%u", originAtom, &originId) == 2) {
...
MxStreamController* SiLoader::OpenStream(const char* p_file)
{
	...
	if (!(controller = Streamer()->Open(p_file, MxStreamer::e_diskStream))) {
		MORTAR_Log("Could not load SI file %s", p_file);
		return nullptr;
	}
	...
	for (si::Core* child : p_core->GetChildren()) {
		if (si::Object* object = dynamic_cast<si::Object*>(child)) {
			if (object->type() != si::MxOb::Null) {
				...
				if ((directive = MORTAR_strstr(extra.c_str(), "StartWith:"))) {
					if (MORTAR_sscanf(directive, "StartWith:%255[^:;]%*[:;]%u", atom, &id) == 2) {
						startWith.emplace_back(
				...
				if ((directive = MORTAR_strstr(extra.c_str(), "RemoveWith:"))) {
					if (MORTAR_sscanf(directive, "RemoveWith:%255[^:;]%*[:;]%u", atom, &id) == 2) {
						removeWith.emplace_back(
							StreamObject{MxAtomId{atom, e_lowerCase2}, id},
							StreamObject{p_atom, object->id_}
						);
				...
				if ((directive = MORTAR_strstr(extra.c_str(), "Replace:"))) {
					if (MORTAR_sscanf(directive, "Replace:%255[^:;]%*[:;]%u", atom, &id) == 2) {
						replace.emplace_back(
							StreamObject{MxAtomId{atom, e_lowerCase2}, id},
							StreamObject{p_atom, object->id_}
						);
				...
				if ((directive = MORTAR_strstr(extra.c_str(), "Prepend:"))) {
					if (MORTAR_sscanf(directive, "Prepend:%255[^:;]%*[:;]%u", atom, &id) == 2) {
						prepend.emplace_back(
							StreamObject{p_atom, object->id_},
							StreamObject{MxAtomId{atom, e_lowerCase2}, id}
						);
				...
				if ((directive = MORTAR_strstr(extra.c_str(), "FullScreenMovie"))) {
					fullScreenMovie.emplace_back(StreamObject{MxAtomId{atom, e_lowerCase2}, id});
				}

				if ((directive = MORTAR_strstr(extra.c_str(), "Disable3d"))) {
